...
class Classname extends Stringable
{
    ...
    public function toRelationshipCode($type, Tree $tree, Model $model)
    {
        ...
        if ($type === 'morphTo') {
            $method_content = sprintf('$this->%s()', $type);
        } elseif ($type === 'morphMany' || $type === 'morphOne') {
            $relation = Str::lower(Str::singular($column_name)) . 'able';
            $method_content = sprintf('$this->%s(%s::class, \'%s\')', $type, $fqcn, $relation);
        } elseif (!is_null($key)) {
            $method_content = sprintf('$this->%s(%s::class, \'%s\', \'%s\')', $type, $fqcn, $column_name, $key);
        } elseif (!is_null($class) && $type === 'belongsToMany') {
            $method_content = sprintf('$this->%s(%s::class, \'%s\')', $type, $fqcn, $column_name);
            $column_name = $class;
        } else {
            $method_content = sprintf('$this->%s(%s::class)', $type, $fqcn);
        }
        ...
        return [$method_name, $method_content];
    }
